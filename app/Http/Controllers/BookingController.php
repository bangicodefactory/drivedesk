...
class BookingController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create booking')) {
            ...
            // Load every driver for the tenant (ordered by name for the picker).
            // The driver SearchableSelect filters client-side, so a capped slice
            // made older drivers unfindable once a tenant had >500 of them
            // (BAN-266). Server-side search is the follow-up for larger scale.
    ...
    public function edit($id)
    {
        if (\Auth::user()->can('edit booking')) {
            ...
            // All drivers for the tenant (ordered by name); see create() — a
            // capped slice made older drivers unfindable in the picker (BAN-266).
            $drivers = User::where('parent_id', parentId())->where('type', 'driver')->orderBy('name')->get()->pluck('name', 'id');
