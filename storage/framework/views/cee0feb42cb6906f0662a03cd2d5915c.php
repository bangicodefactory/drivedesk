                        ...
                        <table class="table table-striped" id="all-reminders-table">
                            ...
                            <tbody>
                                ...
                                <tr class="reminder-row" data-status="<?php echo e($reminder->status); ?>">
                                    ...
                                    <td>
                                        <span class="text-truncate" style="max-width: 150px; display: inline-block;" title="<?php echo e($reminder->note); ?>">
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit reminder')): ?>
                                                <a href="<?php echo e(route('reminder.edit', $reminder->id)); ?>" class="btn btn-sm btn-outline-primary" title="<?php echo e(__('Modifier')); ?>">
                                            ...
                                            <?php if($reminder->status !== 'completed'): ?>
                                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit reminder')): ?>
                                                    <button class="btn btn-sm btn-outline-success" onclick="markAsCompleted(<?php echo e($reminder->id); ?>)" title="<?php echo e(__('Marquer comme terminé')); ?>">
                                            ...
                                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('delete reminder')): ?>
                                                <button class="btn btn-sm btn-outline-danger" onclick="deleteReminder(<?php echo e($reminder->id); ?>)" title="<?php echo e(__('Supprimer')); ?>">
...
function updateUrgentRemindersTable(reminders) {
    ...
    if (reminders.length === 0) {
        tbody.append('<tr><td colspan="7" class="text-center text-muted"><?php echo e(__("Aucun rappel urgent")); ?></td></tr>');
        return;
    }
...
function updateReminderStatuses() {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.success) {
                showNotification(`<?php echo e(__("Statuts mis à jour avec succès")); ?>. ${data.updated} <?php echo e(__("rappels modifiés")); ?>.`, 'success');
                loadDashboardData();
