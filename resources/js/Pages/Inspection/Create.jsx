...
// status, repair_status, notes, amount, receipt, incoming_date,
// meter_reading_incoming, types[<id>][type], types[<id>][note]).
// Posts multipart to route('inspection.store'). The zod schema mirrors the
// controller's store() `required` rules for UX only; Laravel validation stays
// authoritative and its errors surface via setError inside useZodForm.
// Props vehicles/status/repairStatus/types match the controller
// compact('vehicles','status','repairStatus','fuelLevel','types').
...
function InspectionCreate({ vehicles = {}, status = {}, repairStatus = {}, types = [] }) {
    ...
    return (
        <div className="space-y-6 p-6">
            <form onSubmit={submit('post', route('inspection.store'), { forceFormData: true })} className="space-y-6">
