            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <div class="col-12">
                                <label for="statusFilter" class="form-label">{{ __('Filtrer par Statut') }}</label>
                                <select id="statusFilter" class="form-control form-control-sm mb-10">
                                    <option value="">{{ __('Tous') }}</option>
                                    <option value="En Retard">{{ __('En Retard') }}</option>
                                    <option value="Urgent">{{ __('Urgent') }}</option>
                                    <option value="À Venir">{{ __('À Venir') }}</option>
                                                    ...
                                                    @if ($reminder->status !== 'completed')
                                                        @can('edit reminder')
                                                            <button class="btn btn-sm btn-outline-success"
                                                                ...
                                                                title="{{ __('Marquer comme terminé') }}">
    ...
    <script>
        ...
        function updateUrgentRemindersTable(reminders) {
            ...
            if (reminders.length === 0) {
                tbody.append(
                    '<tr><td colspan="7" class="text-center text-muted">{{ __('Aucun rappel urgent') }}</td></tr>');
                return;
            }
        ...
        function updateReminderStatuses() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        showNotification(
                            `{{ __('Statuts mis à jour avec succès') }}. ${data.updated} {{ __('rappels modifiés') }}.`,
                            'success');
                        loadDashboardData();
