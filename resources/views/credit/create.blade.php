{{ Form::open(['url' => route('credit.store'), 'method' => 'post', 'id' => 'credit-create-form']) }}
<div class="modal-body">
    <div class="row">
        <div class="form-group col-md-12">
            ...
            <select name="driver_id" id="credit_driver_id" class="form-control basic-select" required>
                ...
                @foreach ($drivers as $d)
                    <option value="{{ $d->id }}">{{ $d->name }} @if($d->email) ({{ $d->email }}) @endif</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group col-md-6">
            ...
            {{ Form::select('status', $statuses, 'non payé', ['class' => 'form-control basic-select']) }}
        </div>
        <div class="form-group col-md-12">
            {{ Form::label('credit_date', __('Date credit'), ['class' => 'form-label']) }}
            {{ Form::date('credit_date', date('Y-m-d'), ['class' => 'form-control']) }}
        </div>
    </div>
</div>
<div class="modal-footer">
    <button class="btn btn-secondary" type="button" data-bs-dismiss="modal">{{ __('Close') }}</button>
    {{ Form::submit(__('Add Credit'), ['class' => 'btn btn-primary']) }}
</div>
{{ Form::close() }}
